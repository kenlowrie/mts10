...
/*
** This file contains the SQL Database functions for the movie tracking system.
*/

include_once ('sql.logininfo.php');
...
function db_create_table()
{
	...
	$sql = 'CREATE TABLE `info` ('
        ...
        . ' `loanee` VARCHAR(63) NOT NULL, '
        . ' `imdbid` VARCHAR(31) NOT NULL, '
        . ' `fill` VARCHAR(117) NULL,'
        . ' PRIMARY KEY (`uid`),'
        . ' INDEX (`title`, `genre`, `rating`)'
        . ' )'
        . ' ENGINE = InnoDB'
...
function db_load_from_file()
{
	...
	while (!feof($fd)){           // until you reach EOF on the file
		...
		while($token){
          ...
          // Oops if we don't have 6 tokens
		if ($token_count != 6){
...
function db_dump_to_file()
{
	...
	print ("sqlfuncs: wrote $count records to flat file 'movies-out.txt'.<br>\r\n");
...
{
	return "SELECT title, rating, genre, format, loanee, imdbid, uid FROM info";
}
...
/*
** this function processes the output from the search form
** constructing a SQL statement that will get the records
** which match the criteria being set.
*/
function db_set_filter($filter_type, $keywords, $rating, $genre)
{
     switch($filter_type)
     {
     case 'Rating':      // easy, set the rating that they picked
          $query = db_get_select() . " WHERE `rating` = '$rating' " . db_get_order_by();
          break;

     case 'Genre':      // easy, set the rating that they picked
          $query = db_get_select() . " WHERE `genre` = '$genre' " . db_get_order_by();
          break;

     /*
     ** here, we have to form the query so we'll match any of the
     ** keywords that the user specified. go through the keyword
     ** variable and parse out each word separated by a space.
     */
     case 'Keyword':
          if ($keywords != ''){
               $where = ' WHERE ';
               ...
               while($token){
                    $where = $where . "$or `title` LIKE '%$token%' ";
                    $token = strtok(" ");
                    $or = 'OR';
               }

               $query = db_get_select() . $where . db_get_order_by();
          }
          else
             $query = db_get_standard_query();

          break;
     
     case 'Reset':            // user wants to reset the filter
     default:
          $query = db_get_standard_query();
          break;
     }
	db_set_query($query);    // set the query to this new value
}
